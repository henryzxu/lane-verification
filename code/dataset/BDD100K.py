...
class BDD100K(Dataset):
    ...
    train includes train.json[:60k] 60k
    val includes train.json[60k:] 9.863k
    test includes val.json 10k
    """
    TRAIN_SET = ['train_new.json']
    VAL_SET = ['val_new.json']
    TEST_SET = ['test_new.json']
    ...
    def generate_label(self):
        ...
        print("Generating SegData... please be patient")
    ...
    def _gen_label_for_json(self, image_set):
        ...
        with open(json_path) as f:
            ...
            for pic in data:
                ...
                raw_lanes = [item for item in pic["labels"] if item['category'] == "lane"]
                for i in range(len(raw_lanes)):
                    # l = [(x, y) for x, y in zip(label['lanes'][i], label['h_samples']) if x >= 0]
                    l = raw_lanes[i]['poly2d'][0]['vertices']
                    l = [(int(x), int(y)) for x, y in l] # convert to tuple
                    if (len(l)>1):
                        _lanes.append(l)
                ...
                # Eg: "clips/0531/1492626371522989086/20.jpg"
                img_path = "100k/{}/{}".format(image_set, pic['name'])
                ...
                seg_path = "/".join([save_dir, img_path.split("/")[1], img_name[:-3]+"png"])
                if seg_path[0] != '/':
                    seg_path = '/' + seg_path
                if img_path[0] != '/':
                    img_path = '/' + img_path
