    ...
    def postprocess(self, binary_seg_result, instance_seg_result=None,
        ...
        for lane_index, coords in enumerate(lane_coords):
            ...
            if data_source == 'tusimple':
                # ppl_img = np.zeros(shape=(720,1280))
                # ppl_img[coords[:, 1], coords[:, 0]] = 255
                # plt.figure("original")
                # plt.imshow(ppl_img)
                # print(coords.max(axis=0))
                tmp_mask = np.zeros(shape=(720, 1280), dtype=np.uint8)
                tmp_mask[tuple((np.int_(coords[:, 1] * 720 / 288), np.int_(coords[:, 0] * 1280 / 512)))] = 255
                print(tuple((np.int_(coords[:, 1] * 720 / 288), np.int_(coords[:, 0] * 1280 / 512))))
                plt.figure("resize")
                plt.imshow(tmp_mask)
                plt.show()
