...
def hnet_transformation(gt_pts, transformation_coeffcient, name):
    ...
    with tf.variable_scope(name):
        ...
        Y_One = tf.add(tf.subtract(Y, Y), tf.constant(1.0, tf.float32))
        Y_stack = tf.stack([tf.pow(Y, 3), tf.pow(Y, 2), Y, Y_One], axis=1)
        Y_stackT = tf.transpose(Y_stack)
        YTY = tf.matmul(Y_stackT, Y_stack)
        YTY_inv = tf.matrix_inverse(YTY)
        YTY_inv_Y = tf.matmul(YTY_inv, tf.transpose(Y_stack))
        w = tf.matmul(YTY_inv_Y, tf.expand_dims(X, -1))
